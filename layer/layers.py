...
class MergingNet(nn.Module):
    def __init__(self,channel):
        ...
        self.fc1=nn.Linear(3, self.channel, bias=True)
        self.fc2=nn.Linear(3, self.channel, bias=True)
        self.softplus=nn.Softplus()
        
    def forward(self,x,M=256):
        tensor = torch.tensor([64, 256, 1024])
        index = tensor.tolist().index(M)
        M_onehot = torch.zeros(len(tensor)).to('cuda')
        M_onehot[index] = 1
        M_onehot1 = self.fc1(M_onehot)
        M_onehot2 = self.fc2(M_onehot)
        M_onehot1 = self.softplus(M_onehot1).unsqueeze(0).unsqueeze(0)
        M_onehot2 = M_onehot2.unsqueeze(0).unsqueeze(0)
        x = x * M_onehot1 + M_onehot2
        return x
...
class AF(nn.Module):
    def __init__(self,channel):
        super().__init__()
        self.channel=channel
        self.fc1=nn.Linear(self.channel+1,self.channel)
        self.relu=nn.PReLU()
        self.fc2=nn.Linear(self.channel,self.channel)
        self.sigm=nn.Sigmoid()
        self.embedding_layer = nn.Sequential(
            nn.Linear(self.channel+1,self.channel),
            nn.GELU(),
            nn.Linear(self.channel, 2*self.channel),
            nn.GELU(),
        )
        
    def forward(self,x,snr=10):
        z=x
        pooled = torch.mean(x, dim=(2, 3))
        snr_dila=torch.ones(pooled.shape[0],1).to('cuda')
        snr_dila*=snr
        x = torch.cat((snr_dila,pooled),dim=1)
        x = self.embedding_layer(x)
        shift, scale = torch.chunk(x, chunks=2, dim=-1)
        scale = (scale+1).unsqueeze(-1).unsqueeze(-1)
        z = z * scale + shift.unsqueeze(-1).unsqueeze(-1)
        return z
